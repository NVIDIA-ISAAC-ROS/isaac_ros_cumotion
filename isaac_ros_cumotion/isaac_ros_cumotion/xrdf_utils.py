...
def return_value_if_exists(
    input_dict: Dict, key: str, logger, suffix: str = "xrdf"
) -> Any:
...
def convert_xrdf_to_curobo(
    urdf_string: str, input_dict: Union[str, Dict], logger
) -> Dict:
    ...
    if "collision" in input_dict:
        ...
        coll_spheres = return_value_if_exists(
            input_dict["geometry"][coll_name], "spheres", logger
        )
        ...
        if "self_collision" in input_dict:
            if (
                input_dict["self_collision"]["geometry"]
                != input_dict["collision"]["geometry"]
            ):
                logger.error(
                    "self_collision geometry does not match collision geometry"
                )
                raise ValueError(
                    "self_collision geometry does not match collision geometry"
                )
    ...
    default_joint_positions = return_value_if_exists(
        input_dict, "default_joint_positions", logger
    )
    active_config = []
    locked_joints = {}
    ...
    acceleration_limits = return_value_if_exists(
        cspace_dict, "acceleration_limits", logger
    )
